...
void UpdateEntry(BinaryMatrix* M, int row, int col, int c) {
	if(IsMatrixIndex(M,row,col)) {
			...
			printf("Error in UpdateEntry: content must be 0 or 1 ending with a newline and exit the program.\n");
		    exit(-1);
		}
	} else {
		printf("Error in UpdateEntry: index out of bounds ending with a newline and exit the program.\n");
		exit(-1);
	}
}
...
int IsMatrixIndex(BinaryMatrix* M, int row, int col) {
	if(M == NULL) {
		printf("IsMatrixIndex Error: NULL parameter passed ending with a newline and exit the program.\n");
		exit(-1);
	} else {
	    return row < M->num_rows
			&& col < M->num_cols
			&& col >= 0
			&& row >= 0;
	}
